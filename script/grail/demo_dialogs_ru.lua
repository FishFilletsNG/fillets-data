dialogId("dlg-x-poster1", "font_poster", "We recommend subject the enclosed material to thorough tests of holiness. Due to the lack of equipment we performed only the basic tests and the results are more than promising.")
dialogStr("Мы рекомендуем субъекту вложенный материал к полным тестам святости. Из-за нехватки оборудования мы выполнили только базовые тесты, и результаты подают большие надежды.")

dialogId("dlg-x-poster2", "font_poster", "Bombing some of the grails with ultradirty words, we measured sometimes even 3 Santa Clauses, some of them withstanded the circumstances pressure of 0.8 jobs for the period of one minute!")
dialogStr("Бомбя некоторые из чаш Грааля с ультрагрязными словами, мы иногда измеряли трех Санта Клаусов, некоторые из них выдерживали нагрузку: 0.8 заданий в течение одной минуты!")

dialogId("dlg-x-poster3", "font_poster", "It is also possible that we have encountered the whole Holy Service.")
dialogStr("Также возможно, что ты столкнулись со Святой Службой.")

